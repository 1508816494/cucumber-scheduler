...
import com.jd.cucumber.scheduler.core.util.ListHelper;
 ...
 *        system class loader
 *
 * 注意:如果是cucumber-job本身的类,则无论entrust是否为true,都将优先由parent classloader加载.
 *
 * @author Xiaolong Zuo
 * @since 0.9.3
 */
public class ApplicationClassLoader extends URLClassLoader {
    ...
    private static final String CUCUMBER_SCHEDULER_PACKAGE_PREFIX = "com.jd.cucumber.scheduler.";

    private static final String[] CUCUMBER_SCHEDULER_MODULE = new String[]{"core", "api", "scanner", "scheduler", "cluster"};
    ...
    protected boolean isNiubiJobClass(String className) {
        if (className != null) {
            for (String niubiJobModule : CUCUMBER_SCHEDULER_MODULE) {
                if (className.startsWith(CUCUMBER_SCHEDULER_PACKAGE_PREFIX + niubiJobModule)) {
                    return true;
                }
            }
        }
    ...
    public synchronized void addJarFiles(String... jarFilePaths) {
        ...
        for (String jarFilePath : jarFilePaths) {
            ...
            checkAndAddFile(file,jarFilePath);
        }
    }

    protected void checkAndAddFile(File file, Object jarFilePath) {
        if (file.exists()) {
            try {
